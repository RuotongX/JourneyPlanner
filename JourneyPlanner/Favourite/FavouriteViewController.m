...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSArray *keysArray = self.manager.favourites.allKeys;
    NSString  *showKey = keysArray[indexPath.row];
    Favourite *favourite = (Favourite*)self.manager.favourites[showKey];
...
- (void)swipeableTableViewCell:(SWTableViewCell *)cell didTriggerRightUtilityButtonWithIndex:(NSInteger)index {
    ...
    switch (index) {
        case 0:
        {
            ...
            NSArray *keysArray = self.manager.favourites.allKeys;
            NSString  *showKey = keysArray[indexPath.row];
            [self.manager.favourites removeObjectForKey:showKey];
            [self.tableView reloadData];
            break;
        }
        default:
